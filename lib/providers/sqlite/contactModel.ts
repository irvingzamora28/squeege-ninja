...
export class SQLiteContactModel implements IContactModel {
  ...
  async getAll(): Promise<ContactSubmission[]> {
    return this.db
      .prepare(
        `SELECT id, name, email, message, created_at FROM ${ALLSET_CONTACT_SUBMISSIONS_TABLE} ORDER BY created_at DESC`
      )
      .all()
  }
  ...
  async insert(data: Omit<ContactSubmission, 'id' | 'created_at'>): Promise<void> {
    this.db
      .prepare(
        `INSERT INTO ${ALLSET_CONTACT_SUBMISSIONS_TABLE} (name, email, message) VALUES (?, ?, ?)`
      )
