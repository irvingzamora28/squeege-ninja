...
export class SQLiteEmailTemplateDataProvider implements IEmailTemplateDataModel {
  ...
  async getByTemplate(template: string): Promise<EmailTemplateDataInstance[]> {
    return this.db.all<EmailTemplateDataInstance>(
      `SELECT * FROM ${EMAIL_TEMPLATE_DATA_TABLE} WHERE template = ? ORDER BY id DESC`,
      [template]
    )
  }
  ...
  async insert(
    ...
    const stmt = this.db.prepare(
      `INSERT INTO ${EMAIL_TEMPLATE_DATA_TABLE} (template, data, created_at, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`
    )
  ...
  async update(
    ...
    this.db
      .prepare(
        `UPDATE ${EMAIL_TEMPLATE_DATA_TABLE} SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`
      )
      .run(...values)
  }

  async delete(id: number): Promise<void> {
    this.db.prepare(`DELETE FROM ${EMAIL_TEMPLATE_DATA_TABLE} WHERE id = ?`).run(id)
  }
}
